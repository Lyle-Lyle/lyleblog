...
import com.lyle.lyleblog.admin.convert.ArticleDetailConvert;
import com.lyle.lyleblog.admin.model.vo.article.*;
import com.lyle.lyleblog.admin.service.AdminArticleService;
...
public class AdminArticleServiceImpl implements AdminArticleService {
    ...
    public Response publishArticle(PublishArticleReqVO publishArticleReqVO) {
        ...
        // 3. 处理文章关联的分类
        String categoryName = publishArticleReqVO.getCategoryName();

        // 3.1 校验提交的分类是否真实存在 通过分类名称查询
        CategoryDO categoryDO = categoryMapper.selectByName(categoryName);
        if (Objects.isNull(categoryDO)) {
            log.warn("==> 分类不存在, categoryName: {}", categoryName);
    ...
    private void insertTags(Long articleId, List<String> publishTags) {
        ...
        if (CollectionUtils.isEmpty(tagDOS)) {
            ...
            List<String> tagNames = tagDOS.stream().map(tagDO -> String.valueOf(tagDO.getName())).collect(Collectors.toList());
            // 表中已添加相关标签，则需要筛选
            // 通过标签 name 来筛选，包含对应 ID 则表示提交的标签是表中存在的
            existedTags = publishTags.stream().filter(publishTag -> tagNames.contains(publishTag)).collect(Collectors.toList());
            // 否则则是不存在的
            notExistTags = publishTags.stream().filter(publishTag -> !tagNames.contains(publishTag)).collect(Collectors.toList());

            // 补充逻辑：
            // 还有一种可能：按字符串名称提交上来的标签，也有可能是表中已存在的，比如表中已经有了 Java 标签，用户提交了个 java 小写的标签，需要内部装换为 Java 标签
            Map<String, String> tagNameIdMap = tagDOS.stream().collect(Collectors.toMap(tagDO -> tagDO.getName().toLowerCase(), TagDO::getName));
...
//                Long tagId = tagDO.getId();
    ...
    public Response findArticlePageList(FindArticlePageListReqVO findArticlePageListReqVO) {
        ...
        Long size = findArticlePageListReqVO.getPageSize();
...
//        LocalDate endDate = findArticlePageListReqVO.getEndDate();
        ...
        // 执行分页查询
        Page<ArticleDO> articleDOPage = articleMapper.selectPageList(current, size, title);
...
//    }
    ...
    public Response updateArticle(UpdateArticleReqVO updateArticleReqVO) {
        ...
        // 3. 更新文章分类
        String categoryName = updateArticleReqVO.getCategoryName();

        // 3.1 校验提交的分类是否真实存在
        CategoryDO categoryDO = categoryMapper.selectById(categoryName);
        if (Objects.isNull(categoryDO)) {
            log.warn("==> 分类不存在, categoryName: {}", categoryName);
            throw new BizException(ResponseCodeEnum.CATEGORY_NOT_EXISTED);
        }
    ...
    @Override
    public Response findArticleInfo(FindArticleInfoReqVO findArticleInfoReqVO) {
        Long articleId = findArticleInfoReqVO.getArticleId();
        ArticleDO articleDO = articleMapper.selectById(articleId);
        FindArticleInfoRspVO findArticleInfoRspVO = FindArticleInfoRspVO.builder()
                .id(articleId)
                ...
                .build();
        return Response.success(findArticleInfoRspVO);
    }
